 ...
 *
 * On Android the core directory is typically read-only (dladdr() may
 * return a path under /data/app/ or the directory may be restricted by
 * scoped storage / SELinux).  The code therefore also writes the log
 * to the RetroArch save directory as a fallback.  Additionally, all
 * messages are sent to Android logcat via __android_log_print() so
 * they are visible with:  adb logcat -s autoload                */
 ...
 * environ_cb is declared static earlier in this file; an extern declaration
 * here refers to the same entity (C11 6.2.2p4: prior visible declaration
 * with internal linkage determines the linkage).                          */
...
static char autoload_save_dir[AUTOLOAD_MAX_PATH]  = {0};   /* RetroArch save directory (Android fallback) */
...
/* Try to write one line to a log file.  Returns true on success. */
static bool autoload_write_log_line(const char *dir, const char *line)
{
   char logpath[AUTOLOAD_MAX_PATH + 32];
   FILE *fp;
   if (dir[0] == '\0')
      return false;
   snprintf(logpath, sizeof(logpath), "%s%cautoload.log", dir, AUTOLOAD_PATH_SEP);
   fp = fopen(logpath, "a");
   if (!fp)
      return false;
   fprintf(fp, "%s\n", line);
 ...
 * On Android, also sends the message to logcat (always works, no file
 * permissions needed) and tries the RetroArch save directory as a
 * fallback if the core directory is not writable.
...
static void autoload_logf(const char *fmt, ...)
{
   ...
   /* RetroArch log (works on all platforms) */
   if (log_cb)
      log_cb(RETRO_LOG_INFO, "[autoload] %s\n", line);

#ifdef __ANDROID__
   /* Android logcat — always works, no file permissions required.
    * View with:  adb logcat -s autoload */
   __android_log_print(ANDROID_LOG_INFO, "autoload", "%s", line);
#endif

   /* Try writing to the core directory first */
   if (!autoload_write_log_line(autoload_dir, line))
   {
      /* Core directory not writable — try the save directory instead.
       * On Android the save directory (/sdcard/Android/data/com.retroarch/...)
       * is always writable by the RetroArch process. */
      if (autoload_save_dir[0] != '\0')
         autoload_write_log_line(autoload_save_dir, line);
   }
}

/* Trim autoload.log so it keeps only the most recent `keep_runs` runs.
 * Runs are delimited by lines beginning with "--- autoload run ---".
 * Tries the core directory first, then the save directory. */
static void autoload_log_rotate(int keep_runs)
{
   ...
   for (d = 0; d < 2; d++)
   {
      ...
      snprintf(logpath, sizeof(logpath), "%s%cautoload.log", dirs[d], AUTOLOAD_PATH_SEP);
...
static void autoload_try_load_state(void)
{
   ...
   /* On Android, also capture the RetroArch save directory as a fallback
    * for the log file.  The core directory (from dladdr) is often read-only
    * on Android, so we need a writable alternative. */
   {
      const char *sd = NULL;
      autoload_save_dir[0] = '\0';
      if (environ_cb && environ_cb(RETRO_ENVIRONMENT_GET_SAVE_DIRECTORY, &sd) && sd && sd[0])
      {
         strncpy(autoload_save_dir, sd, sizeof(autoload_save_dir) - 1);
         autoload_save_dir[sizeof(autoload_save_dir) - 1] = '\0';
      }
   }
...
#ifdef __ANDROID__
      __android_log_print(ANDROID_LOG_WARN, "autoload",
                          "could not resolve core directory");
#endif
      return;
   }
   ...
   if (autoload_save_dir[0] != '\0')
      autoload_logf("save directory : %s (log fallback)", autoload_save_dir);
